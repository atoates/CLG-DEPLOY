  ...
  try {
    ...
    if (data.usage) {
      if (data.usage.total_tokens) {
        usageInfo = ` • ${data.usage.total_tokens} tokens`;
      } else if (data.usage.input_tokens && data.usage.output_tokens) {
        usageInfo = ` • ${data.usage.input_tokens + data.usage.output_tokens} tokens`;
      }
    }
    ...
    header.innerHTML = `
      ...
      <div class="summary-meta">
        <span>${data.alertCount} alerts • ${data.tokenCount} tokens${usageInfo}</span>
